---
title: Simplified Node Joining for EC2
description: How to setup Simplified Node Joining for Teleport nodes on EC2
h1: Simplified Node Joining for EC2
---

This section will explain how to setup Simplified Node Joining for teleport SSH,
Kubernetes, DB, App, and Proxy services running on EC2. Simplified Node Joining
enables you to allow Teleport nodes to join your Teleport cluster from EC2
instances in your AWS account without sharing any secrets.
...
## Step 2/4. Create the Simplified Node Joining Token

Create the following `token.yaml` with an `allow` rule specifying your AWS
account and regions your EC2 instances will run in.
...
Simplified Node Joining can be used for Teleport services running Node, Proxy,
Kube, App, or Db roles. The service must be run directly on an AWS EC2 instance
and must have network access to the AWS EC2 IMDSv2 (enabled by default for most
EC2 instances).

Teleport must be configured on the node with a custom `teleport.yaml` file.
Instead of using `auth_token` in the configuration, use the `join_params`
section with `token_name` matching your token created in Step 2 and `method:
ec2` as shown in the following example config:
...
`aws_iid_ttl` value in the Simplified Node Joining token so that you have time
...
`aws_iid_ttl` value by editing the yaml file and re-running `tctl create -f
token.yaml`.
...
### Configuring Simplified Node Joining for Multiple AWS Accounts
...
When creating the Simplified Node Joining token, you must include an allow rule
for each foreign account and specify the AWS ARN for the foreign
...
spec:
  ...
  - aws_account: "222222222222"
    ...
    - us-west-1
    - us-west-2
    aws_iid: "arn:aws:iam::222222222222:role/teleport-DescribeInstances-role"
  - aws_account: "333333333333"
    aws_regions:
    - us-west-1
    - us-west-2
    aws_iid: "arn:aws:iam::333333333333:role/teleport-DescribeInstances-role"
```
