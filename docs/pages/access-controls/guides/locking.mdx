...
System administrators can disable a compromised user or host
prevent access during cluster maintenance by placing a lock
on a session, user or host identity.

Teleport will reject all active SSH, database and kubernetes connections and
certificate requests matching the lock's target.
